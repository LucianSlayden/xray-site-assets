/* xray-hex-bg.js — Shared background animation layer
   Lightweight 2D canvas: hex grid with particle constellation overlay.
   No Three.js / WebGL required.
   Used by every page. The page must provide:
     <div  id="xr-page-bg"></div>    ← CSS sets background-image
     <canvas id="xr-anim-canvas"></canvas>
*/
...
(function () {
  ...
  // ── Hex grid ────────────────────────────────────────────────────────
  // Sparse outlines of flat-top hexagons covering the viewport.
  var HEX_R = 52;       // circumradius of each hex
  var hexCells = [];    // {cx, cy, pulse, phase, speed, lit}

  function hexCorners(cx, cy, r) {
    var pts = [];
    for (var i = 0; i < 6; i++) {
      var a = (Math.PI / 180) * (60 * i);   // flat-top: start at 0°
    ...
    for (var c = 0; c < cols; c++) {
      for (var r = 0; r < rows; r++) {
        ...
        hexCells.push({
          ...
          lit:   Math.random() < 0.04,   // ~4% start highlighted
          litT:  0,                       // lifetime of current highlight
        });
      }
    }
  }
  ...
  function Particle() {
    ...
    this.gold  = Math.random() > 0.4;   // 60% gold, 40% sky
  ...
  // ── Draw ─────────────────────────────────────────────────────────────
  // Randomly light up a new hex occasionally
  var LIT_INTERVAL = 40;   // frames between new highlights (at 60fps ≈ 0.67s)
  var litTimer = 0;
  ...
  function draw() {
    ...
    for (var i = 0; i < hexCells.length; i++) {
      ...
      if (cell.lit) {
        cell.litT++;
        // Fade in over 40 frames, hold 60, fade out 40
        var lt = cell.litT;
        var al;
        ...
        if (al > 0) {
          ...
          ctx.closePath();
          ctx.strokeStyle = 'rgba(232,185,99,' + al + ')';
          ctx.lineWidth = 1.2;
          ctx.stroke();

          // Inner fill glow
          ctx.fillStyle = 'rgba(232,185,99,' + (al * 0.06) + ')';
          ctx.fill();
        }
      } else {
        // Ambient base outline — very subtle
        var baseAl = 0.045 + 0.015 * Math.sin(cell.phase + t * cell.speed);
        ctx.beginPath();
        ctx.moveTo(pts[0][0], pts[0][1]);
        for (var p = 1; p < 6; p++) ctx.lineTo(pts[p][0], pts[p][1]);
        ctx.closePath();
        ctx.strokeStyle = 'rgba(111,181,224,' + baseAl + ')';
        ctx.lineWidth = 0.5;
        ctx.stroke();
      }
    }
    ...
    for (var i = 0; i < N_PARTICLES; i++) {
      for (var j = i + 1; j < N_PARTICLES; j++) {
        ...
        if (d2 < MAX_CONNECT * MAX_CONNECT) {
          var d   = Math.sqrt(d2);
          var al  = (1 - d / MAX_CONNECT) * 0.18;
          var col = (pi.gold && pj.gold) ? '232,185,99'
                  : (!pi.gold && !pj.gold) ? '111,181,224'
                  : '172,190,168';
          ctx.beginPath();
